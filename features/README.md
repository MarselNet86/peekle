...
![The island as the attachment strip](shots/attach-strip.png)

![The same, in Russian](shots/attach-strip-ru.png)

The plus under the field raises the macOS file panel, and the panel came up
behind the island: it is an ordinary window of this app, and the island is a
panel above every one of them. Worse, an open island takes every click in its
720 by 560 window, so the top of the dialog — sidebar, search field, path bar —
could not be pressed at all. The island now runs down to one line while files
are picked: a document sign, how many files are already on the message, and the
way back. Everything outside that line goes back to whatever is underneath. The
strip stays after the dialog closes, because a set is usually picked in more
than one go, and the button or ⌘1 from any application opens the island back up.
Sending the message opens it too. A question keeps the digits when one stands:
⌘1 answers the question, and the strip waits for the button.
...
![The way out, after the gear](shots/quit-button.png)

![The quit question](shots/quit-panel.png)

![The quit question, in Russian](shots/quit-panel-ru.png)

![On its way out after Yes](shots/quit-leaving.png)

Peekle can be quit without Activity Monitor. ⌥⌘Q from anywhere, or the power
button after the gear, opens the island just enough to ask: a sign, one line
and two buttons, Yes dark and No white. No, Escape or a click beside the shape
put the island back where it was; Yes folds it away, settles every hook still
waiting, and the app ends a moment later.
...
Nothing new on screen — the same hop as v78, in the same strip
([shots/compact-hop-frames.png](shots/compact-hop-frames.png)) — but it plays
when it is supposed to now.

A class that is already on the element starts no animation. So a second change
of state inside the 620ms of a hop — a compact refused a moment after it
started, a question answered on one card while another ends — swapped the
colour with nothing seen to move, which is the one thing the hop exists to
prevent. The class comes off for a frame before it goes on again.

And no hop is asked for into a state that draws no strokes: `working` swaps
them for the spinner, `waiting` for the question mark, and each of those swaps
announces itself. A hop behind a glyph that is not on screen is a promise the
mark cannot keep.

The route's own rule about a compact — while one runs the turn's working line
stands down, so there is exactly one clock — had no test that could fail: the
unit test pinned `feedRows` with a hardcoded argument, and deleting the clause
from the route broke nothing. It has one now, in the e2e that renders the
route against a stub, because that is the only place the rule lives.
